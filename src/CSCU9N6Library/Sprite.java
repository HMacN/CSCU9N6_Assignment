...
public class Sprite {
    ...
    /**
     ...
     Negative values of scy will flip along the horizontal axis. The
     flipping
    ...
    /**
     Draws the bounding circle of this sprite using the graphics object 'g'
     and
     the currently selected foreground colour.
     */
...
// ensure shifted left and up when flipped due to scaling.
        ...
        transform.translate(Math.round(x)+shiftx+xoff,Math.round(y)
                +shifty+yoff);
        transform.scale(xscale,yscale);
        transform.rotate(rotation,getImage().getWidth(null)/2,getImage().getHeight(null)/
                2);
